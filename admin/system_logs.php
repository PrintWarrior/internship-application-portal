...
// Admins can view all logs except actions performed by superadmins.
$adminLogVisibilityCondition = "(l.user_id IS NULL OR u.user_type IS NULL OR u.user_type != 'superadmin')";
...
$stmt = $pdo->query("
    ...
    WHERE $adminLogVisibilityCondition
...
$todayStmt = $pdo->prepare("
    ...
    FROM system_logs l
    LEFT JOIN users u ON l.user_id = u.user_id
    WHERE DATE(l.created_at) = ? AND $adminLogVisibilityCondition
");
$todayStmt->execute([$today]);
$todayCount = $todayStmt->fetchColumn();

$uniqueUsersStmt = $pdo->query("
    SELECT COUNT(DISTINCT l.user_id)
    FROM system_logs l
    LEFT JOIN users u ON l.user_id = u.user_id
    WHERE l.user_id IS NOT NULL AND $adminLogVisibilityCondition
");
...
<div class="wrapper">
   ...
   onclick="return confirm('Are you sure you want to clear logs? This action cannot be undone.');">
   Clear
</a>
            </div>
        </div>
